...
function esMayorDeEdad (edad: number):any {
    //(edad >= 18)? true: false;
    if(edad >=18) {
        return true;
    } else {
...
//Mejor respuesta 
function esMayorDeEdad2(edad: number): boolean {
    return edad>=18;
}
...
function obtenerPromedio(n1: number, n2: number, n3: number): number | void {
...
function obtenerPromedio2( n1: number, n2: number, n3: number): number{
